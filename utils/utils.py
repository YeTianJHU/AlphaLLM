...
from utils.wizard_math_utils import *
import utils.mammoth_utils as mammoth_utils
...
def format_llama2(query):
    return f"<s>" + f"{B_INST} {B_SYS + DEFAULT_SYSTEM_PROMPT + E_SYS + query} {E_INST} "

def extract_math_reference_answer(answer):
    return strip_string(remove_boxed(last_boxed_only_string(answer)))

def extract_math_generated_answer(answer):
    model_answer_lines = reversed(answer.split("\n"))
...
def extract_math_generated_answer_mammoth(answer):
    dataset = "math"
    answer = mammoth_utils.answer_clean(dataset, get_seperation_trigger(dataset), answer)
    return answer